...
public class ConnectionClass {
    ...
    public ArrayList<CarInfoClass> getInitialCoordinates(String methodName, int rowNum, int branchId) {
    ...
    public UserDataClass getLoginInfo(String methodName, String userName) {
        boolean isUrlMalformed = false;
        URL url = null;
        try {
            url = new URL("http://10.1.11.134/myApp/Webservice1.asmx?op=" + methodName);
        ...
        if (!isUrlMalformed) {
            try {
                ...
                httpURLConnection.setRequestMethod("POST");
                httpURLConnection.setRequestProperty("Content-Type", "text/xml; charset=utf-8");
                httpURLConnection.setRequestProperty("SOAPAction", "http://tempuri.org/GetLoginInfo");
                String sendData = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                        "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">\n" +
                        "  <soap:Body>\n" +
                ...
                try {
                    ...
                    Document doc = dBuilder.parse(bufferedInputStream);
                    doc.getDocumentElement().normalize();
                    NodeList nodes = doc.getElementsByTagName("LoginData");
                    for (int i = 0; i < nodes.getLength(); i++) {
                        Node currentNode = nodes.item(i);
                        Element eElement = (Element) currentNode;
